...
*/
			...
			if( $this->input->post('fields') != null ) {
				...
				// DB ADD START
				// USE FOR DEBUGGING... this insert statement will allow a message to show up in MYSQL
				// showing what the sql statement that's failing was.
				// In MySQL " SET GLOBAL general_log = "ON" "
				// then in /var/lib/mysql there will be a file called: "ip-XX-XX-XX-XX.log" which will
				// show all the (valid) SQL statements that the DB is running.
				//$query = "INSERT INTO query_log_table (QUERY) VALUES (\"".$table."::".$sql."\")";
				//$database->query($query);

				// text values can't have default strings
				$replace_pattern = '/text default \'\'/';
				$replace_string = 'text';
				$sql_rep = preg_replace($replace_pattern, $replace_string, $sql);
				// DB ADD END

				// DB REPLACE
				//$query = 'CREATE TABLE IF NOT EXISTS `'.$table.'` ('.$sql.')';
				// DB WITH
				$query = 'CREATE TABLE IF NOT EXISTS `'.$table.'` ('.$sql_rep.')';
				// DB REPLACE END
				echo $query;
				$database->query($query);
